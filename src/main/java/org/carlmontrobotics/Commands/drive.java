...
public class drive extends Command {
   ...
   */
  public drive(Drivetrain drivetrain, DoubleSupplier fwd, DoubleSupplier str, DoubleSupplier rcw,
  ...
  public double[] getRequestedSpeeds() {
    ...
    if (Math.abs(forward) <= JOY_THRESH)
      forward = 0.0;
    else
      forward *= maxForward;
    if (Math.abs(strafe) <= JOY_THRESH)
      strafe = 0.0;
    else
      strafe *= maxStrafe;
    if (Math.abs(rotateClockwise) <= JOY_THRESH)
      rotateClockwise = 0.0;
    else
  ...
  public boolean hasDriverInput() {
    return Math.abs(fwd.getAsDouble()) > JOY_THRESH
        || Math.abs(str.getAsDouble()) > JOY_THRESH
        || Math.abs(rcw.getAsDouble()) > JOY_THRESH;
  }
